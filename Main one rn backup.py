...
class MainApp(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        # self.backs = [self.findChild(QFrame, f"detail_frame_{i}") for i in range(1, 13)]  # large transparent box behind names and prices
        ...
        # self.main_background = self.findChild(QLabel, "main_background")  # background photo XXX not an object anymore
        self.order_buttons = [self.findChild(QPushButton, f"order_btn_{i}") for i in range(1, 13)]  # order buttons
        self.detail_frames = [self.findChild(QFrame, f"details_frame_{i}") for i in
                              range(1, 13)]  # Frames that the names and prices frames are in
        ...
        for i in range(0, 12):
            self.inputs[i].setCompleter(self.completer_1)  # applies the QCompleters to the QLineEdits
